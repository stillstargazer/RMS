...
public class Reagent 
{
	...
	public Reagent(String str)
	{
		...
		purity=st[3];
		specification=st[4];
		if (st[5].equals("")) number=-1;
		else number=Integer.parseInt(st[5]);
		manufacturer=st[6];
		if (st[7].equals("")) date=-1;
		else date=Integer.parseInt(st[7]);
		if (st[8].equals("")) cabinet=-1;
		else cabinet=Integer.parseInt(st[8]);
	}
